...
#
# Copyright (C) 2026 The checkmk-avocent-snmp authors
...
def check_versions(root: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    if found is not None and found != wanted:
        problems.append(f"pyproject.toml version is {found}, package.manifest says {wanted}")
    return problems
...
def _tar_info(name: str, source: Path, size: int, mtime: int) -> tarfile.TarInfo:
    ...
    # keep the executable bit if a file has one, drop the rest
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the avocent_snmp .mkp")
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="build even if the pyproject.toml version differs",
    )
